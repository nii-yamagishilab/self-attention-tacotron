...
class DatasetSource:
    ...
    def _prepare_source(self, source):
        def convert(inputs: PreprocessedSourceData):
            source = inputs.phone if self.hparams.source == 'phone' else inputs.source
            source_length = inputs.phone_length if self.hparams.source == 'phone' else inputs.source_length
            text = inputs.phone_txt if self.hparams.source == 'phone' else inputs.text
            return SourceData(inputs.id, inputs.key, source, source_length, text)
    ...
    def half_precision(self):
        def convert(s: SourceData, t: MelData):
            return s, MelData(
                id=t.id,
                key=t.key,
                mel=tf.cast(t.mel, dtype=tf.float16),
                mel_width=t.mel_width,
                target_length=t.target_length,
                done=tf.cast(t.done, dtype=tf.float16),
                spec_loss_mask=tf.cast(t.spec_loss_mask, dtype=tf.float16),
                binary_loss_mask=tf.cast(t.binary_loss_mask, dtype=tf.float16))

        return self.apply(self.dataset.map(convert), self.hparams)

    def move_mel_to_source(self):
        def convert(s: SourceData, t: MelData):
